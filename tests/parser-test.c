...
test_parse_integer_literal(void)
{
  ...
  assert_eq(
    nodes[0]->kind, HONEY_AST_COMPTIME_DECL, "should be comptime declaration");
  assert_str_eq(
    nodes[0]->data.comptime_decl.name, "answer", "name should be answer");
...
test_parse_addition(void)
{
  ...
  assert_eq(
    value->kind, HONEY_AST_BINARY_OP, "value should be binary operation");
  assert_eq(
  ...
  assert_eq(
    value->data.binary_op.left->data.int_literal, 2, "left value should be 2");
  assert_eq(value->data.binary_op.right->data.int_literal,
            3,
...
test_parse_multiplication(void)
{
  ...
  assert_eq(
    value->data.binary_op.left->data.int_literal, 4, "left should be 4");
  assert_eq(
    value->data.binary_op.right->data.int_literal, 5, "right should be 5");
...
test_parse_operator_precedence(void)
{
  ...
  assert_eq(value->data.binary_op.left->kind,
            ...
            "left should be int");
  assert_eq(
    value->data.binary_op.left->data.int_literal, 2, "left should be 2");
...
test_parse_parenthesized_expression(void)
{
  ...
  assert_eq(left->kind, HONEY_AST_BINARY_OP, "left should be binary operation");
  assert_eq(
    left->data.binary_op.op, HONEY_BINARY_OP_ADD, "left should be addition");

  // right should be 4
  assert_eq(
    value->data.binary_op.right->data.int_literal, 4, "right should be 4");
...
test_parse_comptime_declaration_with_type(void)
{
  ...
  assert_str_eq(
    nodes[0]->data.comptime_decl.explicit_type, "i32", "type should be i32");
...
test_parse_function_no_params(void)
{
  ...
  assert_str_eq(
    nodes[0]->data.func_decl.name, "get_value", "name should be get_value");
  assert_eq(
    nodes[0]->data.func_decl.param_count, 0, "should have 0 parameters");
  assert_str_eq(
    nodes[0]->data.func_decl.return_type, "i32", "return type should be i32");
  assert_not_null(nodes[0]->data.func_decl.body, "should have body");
  assert_eq(nodes[0]->data.func_decl.body->kind,
            HONEY_AST_BLOCK,
...
test_parse_function_with_params(void)
{
  ...
  assert_eq(
    nodes[0]->data.func_decl.param_count, 2, "should have 2 parameters");
  ...
  assert_str_eq(
    param1->data.name.identifier, "a", "first param name should be a");
  assert_str_eq(
    param1->data.name.type, "i32", "first param type should be i32");

  // check second parameter
  struct honey_ast_node* param2 = nodes[0]->data.func_decl.params[1];
  assert_str_eq(
    param2->data.name.identifier, "b", "second param name should be b");
  assert_str_eq(
    param2->data.name.type, "i32", "second param type should be i32");
...
test_parse_test_declaration(void)
{
  ...
  assert_eq(nodes[0]->data.test_decl.body->kind,
            HONEY_AST_BLOCK,
            "body should be block");
...
test_parse_variable_declaration(void)
{
  ...
  assert_eq(
    body->data.block.statement_count, 1, "block should have 1 statement");
...
test_parse_assignment(void)
{
  ...
  struct honey_ast_node* body = nodes[0]->data.test_decl.body;
  assert_eq(
    body->data.block.statement_count, 2, "block should have 2 statements");
  ...
  assert_str_eq(
    assign_stmt->data.assignment.name, "x", "assignment target should be x");
...
test_parse_return_statement(void)
{
  ...
  assert_eq(
    ret_stmt->kind, HONEY_AST_RETURN_STMT, "should be return statement");
...
test_parse_defer_statement(void)
{
  int count;
  struct honey_ast_node** nodes =
    parse_source("test_defer :: test { x: i32 = 5 defer x = 10 }", &count);
  ...
  // deferred statement
  assert_eq(body->data.block.deferred_count,
            1,
            "block should have 1 deferred statement");
  struct honey_ast_node* defer_stmt = body->data.block.deferred[0];
  assert_eq(
    defer_stmt->kind, HONEY_AST_DEFER_STMT, "should be defer statement");
  assert_not_null(defer_stmt->data.defer_stmt.statement,
...
{
  int count;
  struct honey_ast_node** nodes = parse_source("result :: get_value()", &count);
  ...
  assert_eq(value->data.call_expr.argument_count, 0, "should have 0 arguments");
...
test_parse_function_call_with_args(void)
{
  ...
  assert_eq(value->data.call_expr.argument_count, 2, "should have 2 arguments");

  // check first argument
  struct honey_ast_node* arg1 = value->data.call_expr.arguments[0];
  assert_eq(
    arg1->kind, HONEY_AST_LITERAL_INT, "first arg should be int literal");
  assert_eq(arg1->data.int_literal, 5, "first arg should be 5");

  // check second argument
  struct honey_ast_node* arg2 = value->data.call_expr.arguments[1];
  assert_eq(
    arg2->kind, HONEY_AST_LITERAL_INT, "second arg should be int literal");
...
test_parse_empty_block(void)
{
  ...
  assert_eq(
    body->data.block.deferred_count, 0, "should have no deferred statements");
...
test_parse_block_with_multiple_statements(void)
{
  int count;
  struct honey_ast_node** nodes =
    parse_source("multi :: test { x: i32 = 1 y: i32 = 2 x = 3 }", &count);

  assert_not_null(nodes, "parsing should succeed");
  struct honey_ast_node* body = nodes[0]->data.test_decl.body;

  assert_eq(
    body->data.block.statement_count, 3, "block should have 3 statements");
...
{
  int count;
  struct honey_ast_node** nodes = parse_source("value :: some_var", &count);
  ...
  assert_str_eq(
    value->data.name.identifier, "some_var", "identifier should be some_var");
...
test_parse_multiple_declarations(void)
{
  ...
  assert_eq(
    nodes[0]->kind, HONEY_AST_COMPTIME_DECL, "first should be comptime decl");
  assert_eq(
    nodes[1]->kind, HONEY_AST_COMPTIME_DECL, "second should be comptime decl");
